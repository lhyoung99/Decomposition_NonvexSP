...
%% dimension of first- and second-stage decision variables
n1 = size(first_stage.c,1);
n2 = size(train.Q{1},2);

%% parameters of outer loop
feastol = 1e-6; feastol_abs = 1e-3;
opttol = 1e-6; opttol_abs = 1e-3;
outer_maxiter = 2000;
x0 = zeros(n1, 1);
sample_growth_rate = 50;
plotyes = 0;
...
if isfield(options,'sample_growth_rate'); sample_growth_rate = options.sample_growth_rate; end


%% parameters of inner loop
clear param
...
if isfield(options,'opttol'); param.opttol = opttol; end
if isfield(options,'opttol_abs'); param.opttol_abs = opttol_abs; end


%% scaling factors of the termination test
...
%% Outer loop
x = zeros(n1, outer_maxiter); x(:,1) = x0;
obj = zeros(1, outer_maxiter); %obj(1) = Value(first_stage,test,A,Aeq,x0);
KKT_OptErr = zeros(1, outer_maxiter);

fprintf('====================================================================\n');
fprintf('      Sampling-based Decomposition via Partial Moreau Envelope      \n');
fprintf('====================================================================\n\n');
...
for nu = 1:outer_maxiter
    ...
    % fix sample size
    sample_size = min(sample_growth_rate * nu, train.num);
    
    % call the inner loop for the fixed sample size
    tstart = clock;
    [x(:, nu + 1), FeasErr, OptErr, output] = Inner_DPME(param, first_stage, train, test, sample_size, A, Aeq, x(:, nu));
    obj(nu + 1) = output.obj;
    ...
    fprintf(' %3.2e  %3.2e       %3.2e     %3.2e   %3.4e', FeasErr, OptErr, FeasErr/tau1, OptErr/output.tau2, obj(nu + 1));
    fprintf(' %11.0f  %5.1f\n', output.i, etime(clock, tstart));
   

    %% check stopping criteria
    feastol_all = min(tau1 * feastol, feastol_abs);
    opttol_all = min(output.tau2 * opttol, opttol_abs);
    ...
    if nu <= 1
        ...
        param.epsilon = max(0.1 * KKTtol, 0.1 * param.epsilon);
    elseif flag > 0
        param.gamma = max([1e-3 * param.gamma, min([0.1 * param.gamma, 0.1/output.prox_wor, 10000 * output.adj_dis])]);
        param.epsilon = 0.3 * param.epsilon;
    elseif flag == 0
        if param.gamma * param.epsilon > output.adj_dis
            param.epsilon = max(0.1 * KKTtol, 0.1 * param.epsilon);
        end
            param.gamma = max([7e-3 * param.gamma, min([0.02 * param.gamma, 0.1/output.prox_wor, 10000 * output.adj_dis])]);
    elseif flag == -1
...
info.time = toc;
info.Outer_iter = nu;
info.Total_iter = inneriter_total;
info.KKT_abs = max(FeasErr, OptErr);
info.KKT_rel = max(FeasErr/tau1, OptErr/output.tau2);
info.obj = obj(nu + 1);
